...
const ExclusiveCard = React.memo(({ item, onClick, priority }: { item: any, onClick: () => void, priority?: boolean }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 space-y-4 p-5">
        <div className="flex justify-between items-start gap-2">
          <div className="min-w-0 flex-1">
            {/* РАЗРЯДКА: tracking-tight + leading-snug для воздуха */}
            <h3 className="text-[15px] font-black uppercase tracking-tight leading-snug break-words">{item.name}</h3>
            <p className="text-[9px] font-bold mt-1.5 opacity-60 truncate">{item.farm || "Private Reserve"}</p>
          </div>
      ...
      <div className="relative z-10 flex justify-between items-end p-5 pt-0 mt-1">
        ...
        <div className="text-right ml-2">
           <p className="text-[8px] font-black uppercase opacity-20 leading-none mb-1">Starting at</p>
           <p className="text-[20px] font-black tracking-tight leading-none" style={{ color: accentColor }}>{displayPrice}฿</p>
        </div>
      </div>
    </div>
  );
...
const ProductRow = React.memo(({ p, onClick, priceRef }: { p: any, onClick: () => void, priceRef: any }) => {
  ...
  return (
    <div onClick={onClick} className="flex items-center gap-3 px-5 py-4 hover:bg-white/5 transition-all cursor-pointer group">
      <div className="flex items-center gap-3 min-w-0 flex-1">
        <div className="w-5 flex justify-center shrink-0">
          {p.badge && <BadgeIcon type={p.badge} />}
        </div>
        {/* РАЗРЯДКА: tracking-tight для читаемости в списке */}
        <span className="text-[11px] font-black uppercase tracking-tight text-white/90 truncate leading-tight flex-1">
          {p.name}
        </span>
      </div>
    ...
    <div className="fixed inset-0 z-[250] flex items-center justify-center bg-black/60 backdrop-blur-xl" onClick={onClose}>
      <div className="w-full max-w-sm h-[85vh] px-4 relative flex flex-col items-center justify-center" onClick={e => e.stopPropagation()}>
        ...
        <div className="w-full h-full rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl bg-black/20">
          <img src={imageUrl} width="400" height="700" className="w-full h-full object-cover" alt="" />
        </div>
      </div>
    </div>
  );
...
function ProductModal({ product, style, onClose, categoryBasePrices }: { product: any, style: any, onClose: () => void, categoryBasePrices?: any }) {
  ...
  const getPromoTip = () => {
    ...
    return { diff: (nextWeight - weight).toFixed(0), nextPpg };
  };
      ...
      <div className="relative w-full max-w-lg bg-[#193D2E] rounded-[3rem] border border-white/10 overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="aspect-square w-full relative bg-black/10">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-[#193D2E] to-transparent">
            {/* РАЗРЯДКА: tracking-tight + leading-tight для воздуха в крупном заголовке */}
            <h2 className="text-4xl font-black uppercase tracking-tight leading-tight" style={{ color: style?.color || '#FFF' }}>{product?.name}</h2>
            <p className="text-[10px] font-black uppercase tracking-[0.4em] mt-3 text-white">
        ...
        <div className="p-8 pt-2 space-y-8 text-white">
          <div className="grid grid-cols-3 gap-6 border-b border-white/5 pb-6">
             <div className="space-y-1.5"><div className="flex items-center gap-1.5 opacity-20"><MapPin size={10}/><span className="text-[7px] font-black uppercase tracking-widest">Farm</span></div><p className="text-[10px] font-bold truncate">{product?.farm || '-'}</p></div>
             <div className="space-y-1.5"><div className="flex items-center gap-1.5 opacity-20"><Leaf size={10}/><span className="text-[7px] font-black uppercase tracking-widest">Taste</span></div><p className="text-[10px] font-bold truncate">{product?.taste || '-'}</p></div>
             <div className="space-y-1.5"><div className="flex items-center gap-1.5 opacity-20"><Wind size={10}/><span className="text-[7px] font-black uppercase tracking-widest">Terps</span></div><p className="text-[10px] font-bold truncate">{product?.terpenes || '-'}</p></div>
          </div>
          ...
          <div className="space-y-6">
              ...
              <div>
                <div className="flex items-baseline gap-2">
                   {isSale && basePriceForWeight && basePriceForWeight > currentPrice && (
                     <span className="text-xl font-black tracking-tight line-through opacity-30">{basePriceForWeight}฿</span>
                   )}
                   <div className="text-4xl font-black tracking-tight">{currentPrice}฿</div>
                </div>
                <div className="text-[9px] font-bold opacity-30 uppercase mt-2">Price per gram: {pricePerGram}฿</div>
              </div>
              <div className="text-[11px] font-black uppercase bg-white/10 px-4 py-1.5 rounded-full mb-1">{weight}g</div>
            </div>

            <div className="px-2">
              <input type="range" min={weights[0]} max={weights[weights.length-1]} step={isEliteProduct ? 0.5 : 1} value={weight} onChange={(e) => setWeight(parseFloat(e.target.value))} className="w-full h-1.5 bg-white/10 rounded-lg appearance-none cursor-pointer accent-white" />
            </div>
              ...
              {weights.map(v => {
                ...
                return (
                  <button key={v} disabled={!hasPrice} onClick={() => setWeight(v)} 
                    className={`py-3.5 text-[10px] font-black rounded-xl border transition-all 
            ...
            {promo && (
              <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 flex items-center gap-3 animate-pulse">
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl" onClick={onClose}>
      <div className="relative w-full max-md bg-[#193D2E] rounded-[2.5rem] border border-white/10 flex flex-col max-h-[85vh] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="p-7 border-b border-white/5 flex justify-between items-center bg-black/10 text-white">
          <div><h2 className="text-xl font-black uppercase tracking-tight">Your Basket</h2><p className="text-[10px] font-bold opacity-30 uppercase tracking-[0.2em] mt-1">{items.length} items</p></div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3 no-scrollbar">
          {items.map((item: any) => (
            <div key={`${item.id}-${item.weight}`} className="flex items-center gap-4 bg-white/5 rounded-2xl p-4 border border-white/5 text-white">
              <div className="w-11 h-11 rounded-lg bg-black/20 flex-shrink-0">
                <img src={getOptimizedImg(item.image, 100)} width="44" height="44" className="w-full h-full object-contain" alt="" />
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="text-[11px] font-black uppercase truncate tracking-tight">{item.name}</h3>
                <p className="text-[9px] opacity-40 font-bold uppercase mt-0.5">{item.weight} • {item.price}฿</p>
              </div>
        ...
        <div className="p-7 bg-black/20 border-t border-white/5 space-y-5">
          <button onClick={handleOperatorContact} className="w-full py-4.5 bg-emerald-400/10 border border-emerald-400/20 rounded-xl flex items-center justify-center gap-3 active:scale-95 transition-all group">
          ...
          <div className="grid grid-cols-4 gap-2.5">
            {CONTACT_METHODS.map(m => (
              <button key={m.id} onClick={() => setMethod(m.id)} className={`flex flex-col items-center gap-2 py-3.5 rounded-xl border transition-all ${method === m.id ? "bg-white text-black border-white" : "bg-white/5 border-white/10 opacity-30 text-white"}`}>
                <m.icon size={16} /><span className="text-[7px] font-black uppercase">{m.label}</span>
              </button>
            ))}
          </div>
          <input type="text" placeholder={CONTACT_METHODS.find(m => m.id === method)?.ph} value={contact} onChange={(e) => setContact(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl py-4.5 px-6 text-[12px] font-bold outline-none focus:border-emerald-400 text-white placeholder:opacity-30" />
          <div className="flex items-center justify-between pt-2 text-white"><p className="text-[10px] font-black uppercase opacity-40">Total Amount</p><p className="text-3xl font-black tracking-tight">{total}฿</p></div>
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#193D2E] text-white p-4 md:p-8 pb-32">
      <header className="max-w-xl mx-auto mb-12 pt-6">
        <div className="flex items-center justify-between mb-10">
           ...
           <div className="flex gap-3">
              <Link href="https://t.me/bshk_phuket" target="_blank" className="p-2.5 bg-white/5 rounded-full border border-white/5 opacity-40 hover:opacity-100 hover:bg-[#229ED9]/20 transition-all"><SendHorizontal size={16} className="text-[#229ED9]"/></Link>
              <Link href="https://bndeliveryphuket.click/wa" target="_blank" className="p-2.5 bg-white/5 rounded-full border border-white/5 opacity-40 hover:opacity-100 hover:bg-[#25D366]/20 transition-all"><MessageCircle size={16} className="text-[#25D366]"/></Link>
              <Link href="https://www.instagram.com/boshkunadoroshku" target="_blank" className="p-2.5 bg-white/5 rounded-full border border-white/5 opacity-40 hover:opacity-100 hover:bg-[#E4405F]/20 transition-all"><Instagram size={16} className="text-[#E4405F]"/></Link>
           </div>
        </div>

        <div className="flex gap-6 mb-12 overflow-x-auto w-full no-scrollbar justify-center">
          {STORY_CONFIG.map((config) => {
            const tableData = stories.find(s => s.id === config.id);
            return (
              <button key={config.id} onClick={() => setActiveStory({ ...config, image: tableData?.image })} className="flex flex-col items-center gap-4 shrink-0">
        ...
        <div className="flex gap-4 w-full px-2">
          <Link href="/concentrates" className="flex-1 py-4.5 rounded-2xl bg-[#a855f7]/10 border border-[#a855f7]/30 font-black uppercase text-[9px] tracking-widest text-[#a855f7 flex items-center justify-center gap-2 active:scale-95 transition-all">
            <Flame size={12} /> Concentrates
          </Link>
          <button className="flex-1 py-4.5 rounded-2xl bg-white/5 border border-white/5 font-black uppercase text-[9px] tracking-widest opacity-30 cursor-not-allowed">Accessories</button>
        </div>
      </header>
          ...
          <>
            {gradeSections.map(({ grade, items, priceRef }) => (
              <div key={grade.id} className="rounded-[1.5rem] overflow-hidden border border-white/10 bg-black/20 shadow-xl">
                <div className="px-5 py-4 flex items-center border-b border-white/5" style={{ backgroundColor: `${grade.color}10` }}>
                  <grade.icon size={16} style={{ color: grade.color }} className="mr-3 shrink-0" />
                  <div className="flex flex-col">
                    <h2 className="text-sm font-black uppercase tracking-tight" style={{ color: grade.color }}>{grade.title}</h2>
                  </div>
                  <div className="flex items-center gap-4 ml-auto mr-1">
                    {[1, 5, 10, 20].map(w => (
                      <div key={w} className="flex flex-col items-center min-w-[32px]">
                        <span className="text-[7px] font-black opacity-30 uppercase mb-0.5">{w}g</span>
                        <span className="text-[11px] font-black tracking-tight text-white">{Math.round(getInterpolatedPrice(w, priceRef.prices))}฿</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] w-full max-w-sm px-4">
          <button onClick={() => setIsCheckoutOpen(true)} className="w-full bg-white/10 backdrop-blur-xl text-white p-5 rounded-[2.5rem] shadow-2xl flex justify-between items-center group active:scale-95 transition-all border border-white/20">
            <div className="flex items-center gap-4">
              ...
              <div className="text-left"><p className="text-[10px] font-black uppercase tracking-widest leading-none opacity-40">Order Now</p><p className="text-[18px] font-black mt-1.5 tracking-tight">{getTotal()}฿ Total</p></div>
            </div>
