...
const ExclusiveCard = ({ item, onClick }: { item: any, onClick: () => void }) => {
  ...
  return (
    <div onClick={onClick} className="relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-black/40 backdrop-blur-md p-6 active:scale-[0.98] transition-all cursor-pointer group shadow-2xl">
      <div className="absolute -top-10 -right-10 w-40 h-40 blur-[50px] opacity-20 transition-opacity group-hover:opacity-40" style={{ background: `radial-gradient(circle, ${accentColor} 0%, transparent 70%)` }}></div>
      ...
      <div className="relative z-10 flex flex-col gap-4">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-[10px] font-bold mt-1 opacity-60">{item.farm || "Private Reserve"}</p>
          </div>
          <div className="bg-white/5 border border-white/10 p-2.5 rounded-2xl shrink-0">
             <Crown size={18} style={{ color: accentColor }} />
          </div>
        </div>

        <div className="aspect-[16/10] w-full bg-black/20 rounded-3xl flex items-center justify-center relative overflow-hidden group-hover:bg-black/30 transition-colors border border-white/5">
            <img src={getOptimizedImg(item.image, 500)} className="h-[85%] object-contain drop-shadow-[0_20px_40px_rgba(0,0,0,0.6)] group-hover:scale-110 transition-transform duration-700" alt="" />
        </div>

        <div className="flex justify-between items-end">
          <div className="flex flex-col gap-2">
            <span className="w-fit px-3 py-1 rounded-full bg-white/5 border border-white/5 text-[9px] font-black uppercase tracking-widest" style={{ color: typeColor }}>
              {item.type}
            </span>
          </div>
          <div className="text-right">
             <p className="text-[8px] font-black uppercase opacity-20 leading-none mb-1">Starting at</p>
             <p className="text-3xl font-black italic tracking-tighter" style={{ color: accentColor }}>{displayPrice}฿</p>
          </div>
        </div>
      </div>
    </div>
...
function CheckoutModal({ items, total, onClose }: { items: any[], total: number, onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    if (!contact) return alert("Введите контакт");
    setIsSending(true);
    try {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl" onClick={onClose}>
      <div className="relative w-full max-w-md bg-[#193D2E] rounded-[2.5rem] border border-white/10 flex flex-col max-h-[85vh] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 bg-black/20 border-t border-white/5 space-y-4 text-white">
          <div className="grid grid-cols-4 gap-2">
          ...
          <input type="text" placeholder={CONTACT_METHODS.find(m => m.id === method)?.ph} value={contact} onChange={(e) => setContact(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl py-4 px-6 text-[12px] font-bold outline-none focus:border-emerald-400 placeholder:opacity-30" />
          <div className="flex items-center justify-between pt-2"><p className="text-[10px] font-black uppercase opacity-40 tracking-widest">Total</p><p className="text-3xl font-black italic tracking-tighter">{total}฿</p></div>
    ...
    <div className="min-h-screen bg-[#193D2E] text-white p-4 md:p-8 pb-32">
      <header className="max-w-xl mx-auto mb-10 pt-4">
        <div className="flex items-center justify-between mb-8">
           <div className="flex items-center gap-4">
              ...
              <h1 className="text-[12px] font-black uppercase tracking-[0.3em] text-white opacity-40 leading-none">Premium Phuket delivery</h1>
           </div>
           <div className="flex gap-2">
              <Link href="https://t.me/bshk_phuket" target="_blank" className="p-2 bg-white/5 rounded-full border border-white/5 opacity-40 hover:opacity-100 transition-all"><SendHorizontal size={16} className="text-[#229ED9]"/></Link>
              <Link href="https://bndeliveryphuket.click/wa" target="_blank" className="p-2 bg-white/5 rounded-full border border-white/5 opacity-40 hover:opacity-100 transition-all"><MessageCircle size={16} className="text-[#25D366]"/></Link>
              <Link href="https://www.instagram.com/boshkunadoroshku" target="_blank" className="p-2 bg-white/5 rounded-full border border-white/5 opacity-40 hover:opacity-100 transition-all"><Instagram size={16} className="text-[#E4405F]"/></Link>
           </div>
        </div>
          ...
          {STORY_CONFIG.map((config) => {
            ...
            return (
              <button key={config.id} onClick={() => setActiveStory({ ...config, image: tableData?.image })} className="flex flex-col items-center gap-3 shrink-0 group">
                <div className="w-16 h-16 rounded-full bg-white/5 border-2 flex items-center justify-center transition-all active:scale-90" style={{ borderColor: `${config.color}40` }}><config.icon size={22} style={{ color: config.color }} /></div>
        ...
        <div className="flex gap-3 w-full px-2">
          <Link href="/concentrates" className="flex-1 py-4 rounded-2xl bg-[#a855f7]/10 border border-[#a855f7]/30 font-black uppercase text-[9px] tracking-widest text-[#a855f7] italic flex items-center justify-center gap-2 active:scale-95 transition-all"><Flame size={12} /> Concentrates</Link>
                      ...
                      {gradeItems.map((p: any) => (
                        <div key={p.id} onClick={() => setSelectedProduct(p)} className="flex items-center gap-3 px-5 py-3 hover:bg-white/5 active:bg-white/10 transition-all cursor-pointer group">
                          ...
                          <div className="flex items-center gap-3 shrink-0 ml-auto">
                             <div className="text-[9px] font-bold opacity-20 italic truncate max-w-[80px]">{p.farm}</div>
                             <span className="text-[8px] font-black uppercase px-1.5 py-0.5 rounded bg-white/5 shrink-0" style={{ color: TYPE_COLORS[p.type?.toLowerCase()] || '#10B981' }}>{TYPE_SHORT[p.type?.toLowerCase()] || 'HYB'}</span>
                          </div>
                        </div>
                      ))}
                    </div>
      ...
      {items.length > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] w-full max-w-sm px-4">
          <button onClick={() => setIsCheckoutOpen(true)} className="w-full bg-emerald-400 text-[#193D2E] p-5 rounded-[2.5rem] shadow-2xl flex justify-between items-center group active:scale-95 transition-all border border-white/20">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-black/10 rounded-full"><ShoppingBag size={20}/></div>
      ...
      {selectedProduct && <ProductModal product={selectedProduct} style={isElite(selectedProduct) ? {color: '#FEC107'} : (GRADES.find(g => g.id === selectedProduct.subcategory) || { color: '#FFF' })} onClose={() => setSelectedProduct(null)} />}
