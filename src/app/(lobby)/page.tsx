  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/95 backdrop-blur-2xl" onClick={onClose}>
      <div className="relative w-full max-w-md bg-[#193D2E] rounded-[2.5rem] border border-white/10 p-8 space-y-6" onClick={e => e.stopPropagation()}>
        ...
        <input 
          type="text" 
          placeholder={method === "telegram" ? "@username или номер" : "Контакт для связи"}
          value={contact}
...
export default function LandingPage() {
  ...
  const [isCheckoutOpen, setIsCheckoutOpen] = React.useState(false); // Состояние для модалки заказа
    ...
    <div className="min-h-screen bg-[#193D2E] text-white p-4 md:p-8 pb-32">
      ...
      {items.length > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] w-full max-w-sm px-4">
          <button 
            onClick={() => setIsCheckoutOpen(true)} // Теперь открываем модалку вместо WhatsApp
