...
import { 
  ArrowLeft, ShoppingBag, Send, Zap, Flame, X, 
  TrendingDown, Sparkles, Percent, Crown, MapPin, 
  Wind, MessageCircle, Instagram, SendHorizontal, 
  Trash2, Headset 
} from "lucide-react"
...
function CheckoutModal({ items, total, onClose }: { items: any[], total: number, onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await fetch(URL, { method: "POST", mode: "no-cors", body: JSON.stringify({ contact, method, orderText: getOrderSummary(), total }) });
  ...
  const handleOperatorContact = () => {
    const text = encodeURIComponent(`Hi! I'm interested in concentrates:\n\n${getOrderSummary()}\n\nTotal: ${total}฿`);
        ...
        <div className="p-6 bg-black/20 border-t border-white/5 space-y-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            {CONTACT_METHODS.map(m => (
              <button key={m.id} onClick={() => setMethod(m.id)} className={`flex flex-col items-center gap-2 py-3 rounded-xl border transition-all ${method === m.id ? "bg-white text-black border-white" : "bg-white/5 border-white/10 opacity-30 text-white"}`}><m.icon size={16} /><span className="text-[7px] font-black uppercase">{m.label}</span></button>
            ))}
          </div>
          ...
          <button onClick={handleSubmit} disabled={isSending || items.length === 0} className="w-full bg-emerald-400 text-[#193D2E] py-5 rounded-2xl font-black uppercase text-[12px] tracking-widest active:scale-95 transition-all disabled:opacity-20 shadow-lg">{isSending ? "Sending..." : "Confirm Order"}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#193D2E] text-white p-4 md:p-8 pb-32">
      <header className="flex items-center justify-between mb-10 pt-4 max-w-4xl mx-auto">
        <Link href="/" className="p-3 bg-white/5 rounded-full border border-white/5 opacity-40 hover:opacity-100 transition-all"><ArrowLeft size={20} /></Link>
      ...
      <div className="max-w-xl mx-auto space-y-6">
        {loading ? (
          ...
          Object.entries(grouped).map(([subCat, subItems]: [string, any]) => {
            ...
            return (
              <div key={subCat} className="rounded-[1.5rem] overflow-hidden border border-white/10 bg-black/20 backdrop-blur-md shadow-xl">
                <div className="px-5 py-3 flex justify-between items-center border-b border-white/5" style={{ backgroundColor: `${color}10` }}>
                  <h2 className="text-sm font-black italic uppercase tracking-tighter" style={{ color }}>{subCat}</h2>
                ...
                <div className="divide-y divide-white/5">
                  {subItems.map((p: any) => {
                    ...
                    return (
                      <div key={p.id} onClick={() => setSelected(p)} className="flex items-center justify-between px-5 py-4 hover:bg-white/5 transition-all group cursor-pointer active:bg-white/10">
                        ...
                        <div className="text-[9px] font-bold opacity-20 italic shrink-0 ml-4">from {priceFrom}฿/g</div>
                      </div>
                    );
                  })}
                </div>
