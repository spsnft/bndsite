...
const getPriceByWeight = (weight: number, p: any) => {
  ...
  // Интерполяция если ползунок на другом значении
...
function CheckoutModal({ items, total, onClose }: { items: any[], total: number, onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl" onClick={onClose}>
      <div className="relative w-full max-w-md bg-[#193D2E] rounded-[2.5rem] border border-white/10 flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar">
          {items.map((item: any) => (
            <div key={`${item.id}-${item.weight}`} className="flex items-center gap-4 bg-white/5 rounded-2xl p-3 text-white">
              <img src={item.photo} className="w-12 h-12 rounded-lg object-contain bg-black/20" alt="" />
              <div className="flex-1">
                <h3 className="text-[11px] font-black uppercase italic">{item.name}</h3>
                <p className="text-[9px] opacity-40">{item.weight} • {item.price}฿</p>
              </div>
            </div>
          ))}
        </div>
        <div className="p-6 bg-black/20 border-t border-white/5 space-y-4">
          <input type="text" placeholder="Telegram @username" value={contact} onChange={(e) => setContact(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl py-4 px-6 text-white text-[12px] outline-none" />
          <div className="flex justify-between text-white font-black italic uppercase"><span>Total:</span><span>{total}฿</span></div>
          <button onClick={handleSubmit} disabled={isSending} className="w-full bg-emerald-400 text-[#193D2E] py-5 rounded-2xl font-black uppercase text-[12px]">{isSending ? "Sending..." : "Confirm order"}</button>
        </div>
      </div>
    </div>
  );
...
function ProductModal({ product, style, onClose }: { product: any, style: any, onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl" onClick={onClose}>
      <div className="relative w-full max-w-lg bg-[#193D2E] rounded-[3rem] border border-white/10 overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="aspect-square w-full relative bg-black/10">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-[#193D2E]">
        ...
        <div className="p-8 pt-0 space-y-6 text-white">
          <div className="grid grid-cols-2 gap-4 border-b border-white/5 pb-4 opacity-40 text-[10px] font-bold uppercase italic">
             <div>Farm: {product.farm || "N/A"}</div>
             <div>Microns: {product.microns || "N/A"}</div>
          </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-2">
              {[1, 5, 10].map(v => (
                <button key={v} onClick={() => setWeight(v)} className={`py-3 text-[10px] font-black rounded-xl border ${weight === v ? "bg-white text-black border-white" : "border-white/10 text-white/40"}`}>{v}g</button>
              ))}
            </div>
            <button onClick={() => { addItem({ ...product, price: currentPrice, weight: `${weight}g` }); onClose(); }} className="w-full py-5 rounded-2xl font-black uppercase text-[12px] bg-white text-[#193D2E] active:scale-95 transition-all">Add to Order</button>
          </div>
        </div>
      </div>
    </div>
...
export default function ConcentratesPage() {
  ...
  React.useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const filtered = data.filter((p: any) => String(p.category || "").toLowerCase().trim() === 'concentrates');
  ...
  return (
    <div className="min-h-screen bg-[#193D2E] text-white p-4 md:p-8 pb-32">
      <header className="flex items-center justify-between mb-10 max-w-4xl mx-auto">
        <Link href="/" className="p-4 bg-white/5 rounded-2xl border border-white/10"><ArrowLeft size={20} /></Link>
        <div className="text-center">
          <h2 className="text-2xl font-black italic uppercase tracking-tighter">Concentrates</h2>
          <p className="text-[9px] font-black opacity-30 uppercase tracking-[0.4em]">Phuket stock</p>
        </div>
      ...
      <div className="max-w-4xl mx-auto space-y-10">
        {loading ? (
          <div className="text-center py-20 opacity-20 font-black uppercase text-xs animate-pulse">Checking storage...</div>
        ) : products.length > 0 ? (
          Object.entries(grouped).map(([subCat, subItems]: [string, any]) => {
            const style = getSubStyle(subCat);
            return (
              <div key={subCat} className="rounded-[2.5rem] overflow-hidden border border-white/10 bg-black/20">
                ...
                <div className="divide-y divide-white/5">
                  {subItems.map((p: any) => (
                    <div key={p.id} onClick={() => setSelected(p)} className="flex justify-between items-center px-6 py-5 hover:bg-white/5 cursor-pointer">
          ...
          <div className="text-center py-20 opacity-30 font-bold uppercase text-[10px]">Sheet is empty</div>
        )}
      </div>

      {items.length > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] w-full max-w-sm px-4">
          <button onClick={() => setIsCheckoutOpen(true)} className="w-full bg-emerald-400 text-[#193D2E] p-5 rounded-3xl shadow-2xl flex justify-between items-center border-4 border-[#193D2E] font-black uppercase italic">
            <span>Basket ({items.length})</span><span>{getTotal()}฿</span>
          </button>
        </div>
      )}
