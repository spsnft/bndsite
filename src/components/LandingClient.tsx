...
const BadgeIcon = React.memo(({ type }: { type: string }) => {
  ...
  switch (type.toUpperCase()) {
    case "NEW": return <div className={`${baseClasses} bg-blue-600 border-blue-400`}><span className="text-[7px] font-black text-white tracking-tighter">NEW</span></div>;
    case "HIT": return <div className={`${baseClasses} bg-orange-600 border-orange-400`}><Flame size={11} className="text-white" /></div>;
    case "SALE": return <div className={`${baseClasses} bg-emerald-600 border-emerald-400`}><Percent size={11} className="text-white" /></div>;
    default: return null;
  }
});
...
const HighlightCard = React.memo(({ item, onClick, priority, hideBadge, isMini }: { item: any, onClick: () => void, priority?: boolean, hideBadge?: boolean, isMini?: boolean }) => {
  ...
  return (
    <div onClick={onClick} className={`relative rounded-[2rem] active:scale-[0.98] transition-all cursor-pointer group flex flex-col overflow-hidden ${isMini ? 'h-[180px]' : 'h-[240px]'}`} style={{ boxShadow: `inset 0 0 0 1px ${accentColor}30`, background: `radial-gradient(circle at 50% 0%, ${accentColor}10 0%, rgba(0,0,0,1) 90%)` }}>
...
// --- MODALS (ProductModal & CheckoutModal) ---
        ...
        <div className="px-6 pb-6 space-y-5">
          {(hasValue(product?.farm) || hasValue(product?.taste, "Sweet, Earthy") || hasValue(product?.terpenes, "Myrcene, Limonene")) && (
            <div className="flex flex-wrap gap-4 border-b border-white/5 pb-3">
               {hasValue(product?.farm) && <div className="space-y-0.5"><div className="flex items-center gap-1 opacity-20"><span className="text-[6px] font-black uppercase tracking-widest">{t.farm}</span></div><p className="text-[9px] font-bold italic truncate text-white">{product.farm}</p></div>}
               {hasValue(product?.taste, "Sweet, Earthy") && <div className="space-y-0.5"><div className="flex items-center gap-1 opacity-20"><Leaf size={8}/><span className="text-[6px] font-black uppercase">{t.taste}</span></div><p className="text-[9px] font-bold italic truncate text-white">{product.taste}</p></div>}
               {hasValue(product?.terpenes, "Myrcene, Limonene") && <div className="space-y-0.5"><div className="flex items-center gap-1 opacity-20"><Wind size={8}/><span className="text-[6px] font-black uppercase">{t.terps}</span></div><p className="text-[9px] font-bold italic truncate text-white">{product.terpenes}</p></div>}
            </div>
          )}
...
function CheckoutModal({ items, total, onClose, t }: { items: any[], total: number, onClose: () => void, t: any }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await fetch(URL, { method: "POST", mode: "no-cors", body: JSON.stringify({ contact, method, orderText: items.map(i => `${i.name} (${i.weight}) x${i.quantity} — ${i.price * i.quantity}฿`).join("\n"), total }) });
    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl" onClick={onClose}>
      <div className="relative w-full max-md bg-[#193D2E] rounded-[2.5rem] border border-white/10 flex flex-col max-h-[85vh] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 bg-black/20 border-t border-white/5 space-y-4">
          ...
          <button onClick={handleSubmit} className="w-full bg-emerald-400 text-[#193D2E] py-5 rounded-2xl font-black uppercase text-[12px] tracking-widest active:scale-[0.97] transition-all shadow-[0_10px_30px_rgba(52,211,153,0.2)]">{t.confirmOrder}</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#193D2E] text-white p-4 pb-32 selection:bg-emerald-500/30">
      <header className="max-w-xl mx-auto pt-6 mb-8">
        <div className="flex items-center justify-between px-2"> 
           ...
           <div className="flex items-center flex-1 justify-end">
              ...
              <button onClick={() => setLang(lang === 'en' ? 'ru' : 'en')} className="ml-8 w-12 h-12 flex items-center justify-center bg-white/5 rounded-2xl border border-white/5 font-black text-[10px] text-emerald-400 active:scale-90 transition-all shrink-0">{lang === 'en' ? 'RU' : 'EN'}</button>
           </div>
        </div>
        <div className="grid grid-cols-2 gap-3 mt-10">
          {[
            ...
            <div key={card.id} className="relative p-5 rounded-[2.2rem] border border-white/5 bg-black/20 flex flex-col items-center justify-center text-center min-h-[80px]">
              <div className="space-y-1"><p className="text-[15px] font-black italic text-white uppercase leading-tight">{card.value}</p><p className="text-[7px] font-black uppercase tracking-[0.2em] text-white/30 leading-tight">{(t as any)[card.titleKey]}</p></div>
      ...
      {items.length > 0 && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] w-full max-w-sm px-6">
          <button onClick={() => setIsCheckoutOpen(true)} className="w-full bg-white/10 backdrop-blur-2xl text-white py-3 px-7 rounded-[2.5rem] border border-white/20 shadow-[0_25px_60px_rgba(0,0,0,0.5)] flex justify-between items-center active:scale-95 transition-all">
            ...
            <div className="flex items-center gap-3 text-white relative z-10 opacity-70"><span className="text-[12px] font-black uppercase tracking-widest italic">{t.basket}</span><Send size={18}/></div>
          </button>
        </div>
      )}
      ...
      {selectedProduct && (
        <ProductModal 
          ...
          t={t}
          style={selectedProduct.category === 'concentrates' 
