...
const ExclusiveCard = React.memo(({ item, onClick, priority }: { item: any, onClick: () => void, priority?: boolean }) => {
  ...
  return (
    <div 
      onClick={onClick} 
      className="relative rounded-[2rem] active:scale-[0.98] transition-all cursor-pointer group flex flex-col h-[260px] overflow-hidden"
      style={{ 
    ...
    >
      ...
      <div className="relative z-10 p-5 pb-0 flex flex-col min-h-0 flex-1">
        ...
        <div className="relative flex-1 min-h-0 w-full flex items-center justify-center">
            <BlurImage 
              ...
              width={240}
              height={240}
              className="max-w-full max-h-full object-contain p-2 drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)] group-hover:scale-110 transition-transform duration-500" 
              alt={item.name} 
            />
        </div>
      </div>

      <div className="relative z-10 flex justify-between items-end p-5 pt-2 shrink-0">
